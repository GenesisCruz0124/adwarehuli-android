...
An app is marked a **Confirmed Culprit** when it has both a meaningful
number of observed redirects *and* a high risk score. The Dashboard surfaces
these with a plain-language explanation, e.g. *"Triggered the browser 5x
and can draw over other apps — likely the source of the ads."*
...
- No VPN or network traffic inspection.
...
## Phase 2 plan: VPN-based domain monitor

A future version is planned to add an optional, local-only VPN service
(`VpnService`) that inspects DNS/SNI of outgoing connections entirely
on-device, to catch redirect chains that don't go through the system
browser (e.g. in-app WebViews) and to identify the specific ad/redirect
domains involved. This will remain fully local: no traffic is ever sent off
the device, and the feature will stay strictly opt-in.
